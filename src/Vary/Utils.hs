...
class Mapped (a :: Type) (b :: Type) (xs :: [Type]) (ys :: [Type]) | a b xs -> ys where
  mapVary :: (a -> b) -> Vary xs -> Vary ys

-- instance {-# INCOHERENT #-} (xs ~ ys) => Mapped a b xs ys where
--   mapVary _fun vary = vary

instance forall a b xs ys. Mapped a b (a ': xs) (b ': xs) where
  {-# INLINE mapVary #-}
  mapVary fun vary =
    case popVary vary of
      Right a -> Vary 0 (unsafeCoerce (fun a))
      Left (Vary tag val) -> Vary (tag + 1) val

-- instance forall a b xs ys x. (Mapped a b xs ys) => Mapped a b (x ': xs) (x ': ys) where
--   {-# INLINE mapVary #-}
--   mapVary fun vary =
--     case popVary vary of
--       Right x -> Vary 0 (unsafeCoerce x)
--       Left vary' ->
--         let (Vary tag' val') = mapVary @a @b @xs @ys fun vary'
--          in unsafeCoerce (Vary (tag' + 1) val')

-- type family Mapped (a :: Type) (b :: Type) (xs :: [Type]) :: [Type] where
--   Mapped a b (a ': xs) = (b ': xs)
--   Mapped a b (x ': xs) = x ': Mapped a b xs
